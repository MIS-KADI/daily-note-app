...
export default function AlarmModal({ activeAlarm, onDismiss, onMarkDone, onSnooze }) {
  useEffect(() => {
    if (activeAlarm) {
        ...
        <div className="p-5 space-y-3 bg-slate-50">
          {isMedicine && (
            <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">ડોઝ:</span>
                <span className="font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded-md">
                  {activeAlarm.dosage || '૧ ગોળી'}
                </span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">ખોરાકનો સંબંધ:</span>
                <span className={`font-bold px-2 py-0.5 rounded-md ${
                  activeAlarm.mealRelation === 'before_food'
                    ? 'bg-amber-100 text-amber-800'
                    : 'bg-emerald-100 text-emerald-800'
                }`}>
                  {activeAlarm.mealRelation === 'before_food' ? '🟢 ભૂખ્યા પેટે (ખાલી પેટે)' : '🟡 જમ્યા પછી'}
                </span>
              </div>
              {activeAlarm.notes && (
          ...
          <div className="pt-2 space-y-2">
            ...
            >
              ...
              {isMedicine ? 'દવા લઈ લીધી (Mark Taken)' : 'કામ પૂરું થયું (Mark Done)'}
            </button>
